...
export function selectAllAdmins(groupId: number) {
  return db.prepare(`select user_id from group_admin where group_id=${groupId}`).pluck().all();
}
export function checkAdminExists(groupId: number, userId: number) {
  return db.prepare(`select group_id from group_admin where user_id = ${userId} and group_id=${groupId}`).pluck().get();
}
...
export function removeAdmin(groupId: number, userId: number) {
  db.exec(`delete from group_admin where user_id=${userId} and group_id=${groupId}`);
}
