    ...
    if (evt.atme) {
        ...
        if (isAdmin) {
            if (commonReg.sleep.test(evt.raw_message)) {
                evt.reply("已暂停");
                let flag=db.prepare(`select * from status where group_id=${evt.group_id}`).pluck().get();
                console.log('是否有group',flag)
                if (flag) {
                    db.exec(`update status set sleep=true where group_id=${evt.group_id}`)
                }else {
                    let sql = db.prepare(`insert into status values(${evt.group_id},true)`)
                    sql.run()
                }
                ...
                evt.reply(["我复活啦!", segment.face(99)]);
                  db.exec(`update status set sleep=false where group_id=${evt.group_id}`)
                return;
            }
        }
