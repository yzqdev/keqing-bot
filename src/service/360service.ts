...
import got from "got";
import type { BaseRes, TypeItem } from "@/interface/wallpaper";

export async function get360Types() {
  let types = (await got("http://cdn.apc.360.cn/index.php?c=WallPaper&a=getAllCategoriesV2&from=360chrome").json()) as { data: TypeItem[] };
...
export async function get360Type(type: string = "风景大片"): Promise<any[]> {
  ...
  let res = (await got(`http://wallpaper.apc.360.cn/index.php?c=WallPaper&a=getAppsByCategory&cid=${typeMap.get(type)}&start=0&count=50&from=360chrome`).json()) as BaseRes;
  return res.data;
}
export async function boot() {
  let res = await got(`http://wallpaper.apc.360.cn/index.php?c=WallPaper&a=getAppsByOrder&order=create_time&start=0&count=50&from=360chrome`).json();
  console.log(res);
}
