...
export async function publicData(offsetIndex: number = 1): Promise<PixivItem[]> {
  console.log("获取数据");
  ...
  //原创插图
  let publicUrl = `https://www.vilipix.com/api/v1/picture/public?limit=18&offset=${offsetIndex * 18}&sort=hot&type=0`;
...
export async function rankData(offsetIndex: number = 1): Promise<PixivItem[]> {
  ...
  let dailyUrl = `https://www.vilipix.com/api/v1/picture/ranking?limit=16&offset=${offsetIndex * 16}&type=0&mode=daily`;
  //原创插图
...
export async function genshinData(pageNum: number = 1) {
  let pixList: PixivItem[] = [];
  let genshinUrl = `https://www.vilipix.com/api/v1/picture/public?limit=30&tags=genshin+impact&sort=new&offset=${30 * pageNum}`;
...
export async function realPixiv() {
  //r18默认为false
  let img = (await got(`https://api.lolicon.app/setu/v2?tag=${commonVar.tag}`).json()) as RealPixiv;
